 ...
 * @author pc
 */
@WebServlet(name = "TraitementController", urlPatterns = {"/TraitementController"})
public class TraitementController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        EmployeService el = new EmployeService();
        
        HttpSession session = request.getSession();
        
        String codeentre = request.getParameter("code");
        
        String code_email = (String) session.getAttribute("code");
        ...
        if (c != null) {
            if (code_email.equals(codeentre)) {
                response.sendRedirect("updatepass.jsp");
            } else {
                response.sendRedirect("traitement.jsp?msg= le code est incorrect!! ");
            }
        }else if (e != null) {
            if (code_email.equals(codeentre)) {
                response.sendRedirect("updatepass.jsp");
            ...
            response.sendRedirect("traitement.jsp?msg= sesion vide!! ");
        }
    }
